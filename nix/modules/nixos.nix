...
in
{
  options.programs.lazytf = {
    ...
    settings = lib.mkOption {
      ...
      default = { };
      description = "Settings to write to /etc/lazytf/config.yaml.";
    };
  };

  config = lib.mkIf cfg.enable {
    environment.systemPackages = [ cfg.package ];
    environment.etc."lazytf/config.yaml".source = yamlFormat.generate "lazytf-config.yaml" (pruneNulls cfg.settings);
  };
}
