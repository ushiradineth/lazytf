...
func (m *Model) registerKeybindHandlers() {
	...
	r.RegisterHandler(keybinds.ActionDriftView, m.handleActionDriftView)
...
func (m *Model) handleActionDriftView(ctx *keybinds.Context) tea.Cmd {
	...
	if m.mainArea.GetMode() == ModeDrift {
		m.mainArea.SetMode(ModeDiff)
		return nil
	}
	if m.plan == nil || len(m.plan.Resources) == 0 {
		return m.toastInfo("No plan loaded for drift analysis")
	}
	m.mainArea.SetDriftContent(BuildDriftAnalysisView(m.plan.Resources))
	if m.panelManager != nil {
		m.panelManager.SetFocus(PanelMain)
	}
	m.updateLayout()
